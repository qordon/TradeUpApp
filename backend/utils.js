...
const itemsLink =
  'https://raw.githubusercontent.com/SteamDatabase/GameTracking-CS2/refs/heads/master/game/csgo/pak01_dir/scripts/items/items_game.txt';
...
async function fileGetError(items) {
  let csgoEnglish = require('./data/csgo_english.json');
  items.setTranslations(csgoEnglish, 'Error');
  let itemsGame = require('./data/items_game.json');
  items.setCSGOItems(itemsGame);
}

async function getTranslations(items) {
  try {
    const filePath = path.join(__dirname, 'data', 'csgo_english.txt');
...
async function updateItems(items) {
  try {
    const filePath = path.join(__dirname, 'data', 'items_game.txt');
    const data = fs.readFileSync(filePath, 'utf8');
