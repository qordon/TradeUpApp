        ...
        <table>
          <thead>
            <tr style="padding-left: 5px;">
              <th @click="sortData('item_name')" style="width: 34%">
                ITEM
                <span v-if="sortKey === 'item_name'">
                  {{ sortOrder === 'asc' ? '▴' : (sortOrder === 'desc' ? '▾' : '↕') }}
                </span>
              </th>
              <th @click="sortData('item_paint_wear')" style="width: 7%">
                FLOAT
              ...
              <th @click="sortData('stickers')" style="width: 20%">
                STICKERS
                <span v-if="sortKey === 'stickers'">
                  {{ sortOrder === 'asc' ? '▴' : (sortOrder === 'desc' ? '▾' : '↕') }}
                </span>
              </th>
              <th @click="sortData('trade_unlock')" style="width: 8%">
                TRADABLE
                <span v-if="sortKey === 'trade_unlock'">
                  {{ sortOrder === 'asc' ? '▴' : (sortOrder === 'desc' ? '▾' : '↕') }}
                </span>
              </th>
              <th @click="sortData('qty')" style="width: 5%; white-space: nowrap;">
                QTY
...
.sticker-container {
  ...
  gap: 0;
  width: 45px; /* match image width to avoid increasing column width */
  height: 45px; /* same as image height to keep row height stable */
  position: relative;
}
...
.sticker-name {
  position: absolute;
  top: calc(100% + 2px); /* render below image without affecting layout */
  left: 50%;
